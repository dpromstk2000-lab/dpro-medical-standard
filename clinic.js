(function () {
  ...
  function status(s) { return `<span class="status ${esc(s)}">${esc(visitLabels[s] || s || '-')}</span>`; }
  ...
  function renderKpis() {
    ...
    document.querySelectorAll('[data-adapter-mode]').forEach(el => { el.textContent = 'DATA SOURCE: ' + API.config.mode.toUpperCase(); });
    document.querySelectorAll('[data-runtime-env]').forEach(el => { el.textContent = API.config.environmentMode.toUpperCase(); });
    document.querySelectorAll('[data-role-hint]').forEach(el => { el.textContent = (API.config.role || 'owner').toUpperCase() + ' UI'; });
  }
  ...
  function renderToday(target) {
    const rows = state.today.map(row => {
      ...
      return `<tr>
        ...
        <td><div class="patient-name">${esc(p.name || row.patientName || '患者')}</div><div class="muted">${esc(patientId)}</div></td>
  ...
  function renderReception(target) {
    target.innerHTML = `<div class="toolbar"><input id="reception-search" style="min-width:320px" placeholder="患者番号・氏名・カナ・電話番号"><button class="btn primary" id="reception-search-btn">検索</button></div>
      <div class="notice">予約あり受付は正式な atomic endpoint <code>POST /api/medical/v1/check-in</code> を使用します。UI側で複数CORE更新を分割実行しません。</div>
  ...
  async function searchAndRender(target, q, withReception) {
    try {
      ...
      target.innerHTML = list.map(p => {
        ...
        return `<div class="patient-card"><div><div class="patient-name">${esc(p.name)} <span class="muted">${esc(p.patientId)}</span></div>
    ...
    } catch (err) { target.innerHTML = `<div class="notice">検索エラー: ${esc(err.message)}</div>`; }
  }
  ...
  function renderWorkflow(target) {
    ...
    target.innerHTML = `<div class="flow">${active.map(s => {
      ...
      return `<div class="flow-col"><div class="flow-title"><span>${esc(visitLabels[s])}</span><span>${visits.length}</span></div>${visits.map(v => {
        ...
        return `<div class="flow-card"><strong>${esc(p.name)}</strong><div class="muted">${esc(p.patientId)} / ${esc(v.workflowStage || '')}</div>
  ...
  function renderQuestionnaires(target) {
    if (!hasPermission('questionnaire.read')) {
      target.innerHTML = '<div class="notice">questionnaire.read 権限がありません。</div>';
      return;
    }
    target.innerHTML = `<div class="table-wrap"><table><thead><tr><th>患者</th><th>状態</th><th>運用要約</th><th>回答時刻</th><th>詳細</th></tr></thead><tbody>${state.questionnaires.map(q => {
      ...
      return `<tr><td><div class="patient-name">${esc(p.name)}</div><div class="muted">${esc(p.patientId)}</div></td><td>${status(q.status)}</td><td>${esc(q.summary || '-')}</td><td>${esc(q.submittedAt || '-')}</td><td><button class="btn small" data-questionnaire-detail="${esc(id)}">権限確認して表示</button></td></tr>`;
  ...
  async function showQuestionnaireDetail(id, button) {
    ...
    try {
      ...
      panel.innerHTML = `<div class="card"><strong>問診詳細</strong><p class="permission-note">一覧では全文を露出せず、questionnaire.read確認後に詳細を取得しています。</p><pre style="white-space:pre-wrap;word-break:break-word">${esc(JSON.stringify(detail, null, 2))}</pre></div>`;
    } catch (err) { panel.innerHTML = `<div class="notice">詳細表示不可: ${esc(err.message)}</div>`; }
  }
  ...
  async function checkIn(appointmentId) {
    ...
    try {
      ...
      await loadData();
    } catch (err) { showError('受付エラー: ' + err.message); }
  ...
  async function advanceVisit(visitId) {
    ...
    try {
      ...
      await loadData();
    } catch (err) { showError('院内進行更新エラー: ' + err.message); }
    finally { state.busy = false; }
  }
  ...
  async function initClinic() {
    ...
    bindGlobalActions();
    try { await loadData(); }
    catch (err) { showError('初期読込エラー: ' + err.message); }
  }
